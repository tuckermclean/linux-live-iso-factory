...
src_install() {
	...
	#   3. zz-fortune.sh last, always — "wisdom must never bury status"
	#
	...
	#     once-guard to it.
	#   - fortune: once per boot via /run/monolith-fortune-shown (not
	#     once per shell — see that file's own header).
	...
	# file in this tree regardless of how high the numeric scheme above
	# grows — see the file's own header for the ASCII-sort reasoning.
	# "Wisdom must never bury status."
