...
src_configure() {
	...
	#
	# LC_ALL format (patched in config.h after configure, below): musl's
	# setlocale(LC_ALL,"") returns a POSITIONAL composite ("C.UTF-8;C;C;C;C;C" —
	# musl only supports UTF-8 for LC_CTYPE, the rest stay C), but perl-cross
	# can't RUN a target probe to learn the format, so it defaults to glibc's
	# name=value assumption (d_perl_lc_all_uses_name_value_pairs=define). At boot
	# that made perl panic in locale.c: "'C.UTF-8;C;C;C;C;C' needs an '=' to split
	# name=value". Undefining PERL_LC_ALL_USES_NAME_VALUE_PAIRS selects locale.c's
	# positional LC_ALL parser (the musl/BSD path). A global LC_ALL=C hammer is
	# deliberately NOT used — it would strip UTF-8 from every tool on the disc;
	# the fix belongs in perl.
	./configure \
		--target="${CHOST}" \
		--prefix=/usr \
		-Dcc="${CHOST}-gcc" \
	...
	# Force PERL_LC_ALL_USES_NAME_VALUE_PAIRS off (musl positional LC_ALL — see
	# the big note above). -Ud_perl_lc_all_uses_name_value_pairs on the configure
	# line did NOT stick (perl-cross accepts the token but its config generation
	# / hints reset the symbol back to define, so perl still panicked at boot:
	# locale.c "'C.UTF-8;C;C;C;C;C' needs an '=' to split name=value"). Patch the
	# generated config.h directly instead — deterministic, independent of
	# perl-cross's variable tracking. The grep guard fails the build loudly if the
	# expected #define line ever changes shape, rather than silently regressing.
	# Best-effort: also flip the source-of-truth in config.sh, so if make ever
	# regenerates config.h from config.sh the result stays correct (no `die` —
	# perl-cross may not track this var in config.sh at all, in which case a
	# regenerated config.h simply omits the #define, which is also what we want).
	if [[ -f config.sh ]]; then
		sed -i -E "s/^d_perl_lc_all_uses_name_value_pairs=.*/d_perl_lc_all_uses_name_value_pairs='undef'/" config.sh || true
	fi
	# Authoritative: patch the generated config.h directly (deterministic,
	# independent of perl-cross's variable tracking). config.h is already newer
	# than config.sh/config_h.SH after configure, so make's timestamp rules won't
	# regenerate over this. The grep guard fails the build loudly if the expected
	# #define line ever changes shape, rather than silently regressing.
	[[ -f config.h ]] || die "config.h not generated by perl-cross configure"
	sed -i -E 's/^#define[[:space:]]+PERL_LC_ALL_USES_NAME_VALUE_PAIRS.*/#undef PERL_LC_ALL_USES_NAME_VALUE_PAIRS/' config.h \
		|| die "failed to patch PERL_LC_ALL_USES_NAME_VALUE_PAIRS in config.h"
	grep -q '^#undef PERL_LC_ALL_USES_NAME_VALUE_PAIRS' config.h \
		|| die "LC_ALL config.h patch did not apply (define line shape changed?)"
