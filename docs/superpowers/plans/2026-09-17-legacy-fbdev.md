...
- [x] Flip these 24 from `# CONFIG_X is not set` to `CONFIG_X=m`:
...
- [x] Set `CONFIG_FB_GEODE=y` (bool menu gate, no object file — see the spec on
      why this is not a Tier-0 addition) and add `CONFIG_FB_GEODE_LX=m`,
      `CONFIG_FB_GEODE_GX=m`, `CONFIG_FB_GEODE_GX1=m`.
- [x] Add the chip sub-options that the drivers are useless without:
...
- [x] Force the DDC traps off — these are `bool`s whose `y` would escalate
...
- [x] Do **not** hand-write the symbols `olddefconfig` is expected to derive
...
- [x] Leave every excluded symbol at `# … is not set`. Do not touch
...
- [x] `git mv configs/overlay/sys-kernel/monolith-kernel/monolith-kernel-6.12.80-r6.ebuild`
      → `…-r7.ebuild` (content unchanged) and regenerate/adjust `Manifest` if
      the repo's flow requires it.
- [x] Bump `configs/portage/versions.lock`:
      `sys-kernel/monolith-kernel:6.12.80-r6:0` → `…-r7:0`.
- [x] Sanity-check the consumers of that pin that strip `-rN`: the
      ...
      assume.
- [x] Check the "Purge stale savedconfig binpkgs" step in `build.yml` — if it
...
- [x] Assert each of the 24 driver symbols is exactly `^CONFIG_<sym>=m$`.
- [x] Assert `CONFIG_FB_GEODE=y` and the three `FB_GEODE_*=m`.
- [x] Assert the eight chip sub-options at their required values.
- [x] Assert the five DDC sub-options are `# … is not set`, with a comment
      naming the reason (`bool=y` → `FB_DDC=y` → `CONFIG_I2C=y`), so a future
      reader doesn't "fix" the test by turning them on.
- [x] Assert `CONFIG_I2C` is **not** `=y` — the actual invariant those five
      lines exist to protect. `=m` is fine and expected; `=y` is the failure.
- [x] Assert every excluded symbol from the spec is still `# … is not set`,
      including `FB_ASILIANT`, `FB_IMSTT`, `FB_VIRTUAL`.
- [x] Re-assert the covenant: `CONFIG_EXTRA_FIRMWARE` must not be set.
- [x] `sh scripts/tests/verify-legacy-fbdev-config.sh` prints `ALL PASS` and
...
- [x] Assert all 27 `.ko` files exist under `$MODDIR`:
...
- [x] Enforce the firmware doctrine: for each module, `modinfo -F firmware` must
...
- [x] Echo the count on success, matching the NIC step's output style.
...
- [x] Add `build_vga_cmd(args)`: `build_bios_cmd(args)` plus
      `["-vga", args.vga_model]`. Raise `BootTestError` if `--vga-model` is
      missing, mirroring `build_nic_cmd`'s guard.
- [x] Add `--vga-model` (default `None`) to the argument parser and register the
      new mode in `MODE_RUNNERS`.
- [x] Add `VGA_MODEL_MODULE = {"cirrus": r"^cirrusfb\b"}` and
      `smoke_vga_model_is_module(child, model)` asserting `lsmod` matches,
      modelled on `smoke_nic_model_is_module`.
- [x] Runner: boot the `serial vga=788` ISOLINUX label as the other BIOS modes
      do, run the reduced smoke suite, then append the `cirrusfb` check.
- [x] Write the docstring to say *why* this proves something: `-vga cirrus` is a
      ...
      the driver.
- [x] Wire a job into `.github/workflows/boot-test.yml` alongside the NIC matrix
...
- [x] Add the set to `docs/kernel-tiers.md` under Tier 2, in the style of the
...
- [x] Note the two mechanism-excluded symbols (`FB_ASILIANT`, `FB_IMSTT`) so
      nobody re-proposes them without arguing the Tier-0 case.
- [x] Record the `FB_TILEBLITTING=y` core addition as the one deliberate
      `bzImage` growth.
- [x] Add a line to `AGENTS.md`'s traps section for the DDC → `CONFIG_I2C=y`
